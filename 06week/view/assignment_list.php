...
<section class="assignment-container">
    ...
    <?php if (!empty($assignments)) : ?>
        <?php foreach ($assignments as $assignment) : ?>
            <div class="assignment-item" style="display:flex; flex-direction: row; gap: 10px; align-items: center;">
                ...
                <form action="." method="post">
                    ...
                    <input type="hidden" name="assignment_id" value="<?= $assignment['ID'] ?>">
                    <button type="submit" class="" onclick="return confirm('Are you sure you want to delete this assignment?')">X</button>
                </form>
            </div>
        <?php endforeach; ?>
    <?php else : ?>
